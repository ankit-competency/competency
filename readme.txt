=== Import-users-to-icontact ===
Contributors: Ankit Gupta, Rahul Gupta
Tags: IContact, icontact, Import, Import User,IContact User, Integration IContact, Import users to IContact
Requires at least: 3.2
Tested up to: 4.8


This Plugin provide to you add users in to IContact Dashboard.

== Description ==

This plugins will add import button into users listing and also admin can import all users to IContact.


== Installation ==

This section describes how to install the plugin and get it working.

e.g.

1. Upload the `Import-users-to-icontact` directory to the `/wp-content/plugins/` directory.
1. Activate the plugin through the 'Plugins' menu in WordPress.



== Frequently Asked Questions ==



== Screenshots ==

1. Sample output
